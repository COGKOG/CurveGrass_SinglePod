...
GrassSystem::GrassSystem(int num)
{
    ...
    for(int i=0; i<array_size; i++){
        // random value of [4, 11]
        randomVal = rand() % (11 - 4 + 1) + 4;
        ...
        for (int k = 0; k < m_numParticles; k++){
            ...
            for (int j = 0; j < 5; j++){
                ...
                for (int i = 0; i < j_array[counter]; i++) {
                    Vector3f pos = Vector3f(l*0.8, i*0.5, k*0.8);
                    state.push_back(pos);
...
vector<Vector3f> GrassSystem::evalF(vector<Vector3f> state)
{
    ...
    for(int j = 0; j < array_size; j++){
        ...
        for (int i = 1; i < j_array[j]; i++) {
            ...
            // curvature force
            if (i > j_array[j]/2.0 && j_array[j] >5){
                force+=expo_force;
                expo_force = expo_force*1.1;
            }
            
            // wind force
            if(wind){
                float fraction = float(rand() / static_cast <float> (RAND_MAX));
                force+=Vector3f(0.0f, 0.0f, 2.0f*(fraction));
            }
